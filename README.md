...
A Redis client for [Milo](https://github.com/milo-language/milo), speaking RESP
directly. No hiredis, no C dependency for the protocol — just the standard
library.
...
fn main(): i32 {
    ...
    // One write, a hundred commands.
    var p = Pipeline.new()
    for i in 0..100 {
        p.add(["INCR", "hits"])
    }
    let replies = db.runPipeline(p)!
    print(replies.len().toString() + " replies in one round trip")
